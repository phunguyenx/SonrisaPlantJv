...
public class GetAllPlantWishListUseCase {
    private final PlantRepository plantRepository;
    private final Application application;
    private List<PlantDto> plants;
    public MutableLiveData<List<PlantDto>> listPlant;
    public GetAllPlantWishListUseCase(PlantRepository plantRepository, Application application) {
        this.plantRepository = plantRepository;
        this.application = application;
    }
    public MutableLiveData<List<PlantDto>> execute(PlantRequestParameters request){
        listPlant = new MutableLiveData<>();
        Call<ListOfPlant> call = plantRepository.getAllPlantWishlist(request);
...
//                        callback.onSuccess();
                    ...
                    Log.e("get all plant wishlist use case"," - > Error    "+ e.getMessage());
//                    callback.onFailure(Constant.MSG_FAIL);
            ...
            public void onFailure(Call<ListOfPlant> call, Throwable t) {
                Log.d("get-plant_wishlist_Call"," - > Error    "+ t.getMessage());
//                callback.onFailure(Constant.MSG_FAIL);
            }
        });
        return listPlant;
